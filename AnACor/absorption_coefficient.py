...
class AbsorptionCoefficient(object):
    def __init__(self, tomo_img_path, ModelFilename, auto_orientation, auto_viewing, logger,li=0,lo=0,
                 cr=0,bu=0,base='li', crop=None, thresholding='mean', angle=0, save_dir="./", pixel_size=0.3,
    ...
    def determine_single_class(self, cls, percent, y_list, x_list, region_back, target, single=True, determine_peaks=False, lower_peak=True):
        ...
        if determine_peaks:
            ...
            bins, edges, patches = self.histogram_in_area(
                roi_cls, percent, cls=cls, number_bins=50, xlim=None, determine_peaks=determine_peaks)  # [0.01,0.03] )
            try:
    ...
    def differet_orientation(self, angle, flat_fielded=None):
        ...
        self.imagemask_overlapping( candidate_img, shifted_mask,"threshold_of_angle_{}_yellow_is_the_projection_of_3d_model".format(angle))
...
class RunAbsorptionCoefficient(AbsorptionCoefficient):
    def __init__(self, tomo_img_path, ModelFilename, auto_viewing, auto_orientation, logger,li=0,lo=0,
                 cr=0,bu=0,base='li',
                 angle=0, save_dir='./', pixel_size=0.3,
                 kernel_square=(15, 15), full=False, offset=0, v_flip=False, h_flip=False,
                 ModelRotate=-90, crop=None, thresholding='triangle', flat_fielded=None):
        super().__init__(tomo_img_path, ModelFilename, angle=angle, logger=logger,li=li,lo=lo,
                        cr=cr,bu=bu,base=base,
    ...
    def run(self,singlecls=False):
        ...
        main_classes = ['li','cr', 'lo']
        main_classes.remove(self.base)

        if singlecls:
            
            roi_othercls = self.determine_single_class(cls=other_cls, percent=percent, y_list=variable_dict_y[other_cls],
                                                            x_list=variable_dict_x[other_cls],
                                                            region_back=variable_region_back[other_cls], target=rate_list[
            ...
            for percent in order:
                ...
                roi_base = self.determine_single_class(cls=self.base, percent=percent, y_list=variable_dict_y[self.base],
                                                    x_list=variable_dict_x[self.base],
                                                    region_back=variable_region_back[self.base], target=rate_list[
                ...
                variable_dict_list[self.base].append(self.coe_li)
