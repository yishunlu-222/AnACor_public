...
__global__ void rt_gpu_python_results(size_t len_coord_list, float *d_result_list, float *d_python_result_list,size_t len_result)
{
...
__global__ void ray_tracing_rotation(const float *d_omega_axis, float *d_omega_list, float *d_kp_rotation_matrix, float *d_raw_xray, float *d_scattering_vector_list, size_t len_result, float *d_rotated_xray_list, float *d_rotated_s1_list)
{
...
// __global__ void rt_gpu_get_face(int *d_face, int *d_coord_list, float *d_rotated_s1, float *d_xray, int x_max, int y_max, int z_max, size_t len_coord_list, int)
__global__ void rt_gpu_get_face(int *d_face, int *d_coord_list, float *d_rotated_s1_list, float *d_rotated_xray_list, int x_max, int y_max, int z_max, size_t len_coord_list, int batch_number)
{
	size_t id = blockIdx.x * blockDim.x + threadIdx.x;
                                   ...
                                   int store_paths, float *h_result_list, int *h_face, float *h_angles, float *h_python_result_list)
{
	...
	printf("len_result %d \n", len_result);
	printf("result_size is %0.3f MB\n", (double)result_size/ (1024.0 * 1024.0));
