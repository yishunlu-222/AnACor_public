...
para_dict={'list_ac_at':list_ac_at, 'list_acsh_at':list_acsh_at, 
           ...
           'list_sh_af':list_sh_af, 'list_sh_at':list_sh_at,}
para_dict_l={'l_ac_at':l_ac_at, 'l_acsh_at':l_acsh_at, 
           'l_acsh_af':l_acsh_af, 'l_ac_af':l_ac_af,
           'l_sh_af':l_sh_af, 'l_sh_at':l_sh_at,}
final_result=[]
final_result_l=[]
base='/dls/i23/data/2023/nr29467-16/processed/Cld/for_anacor/cld_1704_12/'
heading=[]
heading_l=[]
for anom in ['af', 'at']:
        target='sh'
        if anom == 'at':
            single_pth=os.path.join(base)
        else:
            continue
        ...
        for i, dir_pth in enumerate(dataset_list):
            ...
            tmp_pth=os.path.join(single_pth, dir_pth,'xia2-dials-P1')
            data_pth=find_file(tmp_pth, 'SCALE.log'.format(target))
...
target='sh'
anom='at'
for i in range(len(para_dict['list_sh_at'])):
    tmp=[]
    tmp_1=[]
    # for anom in ['af', 'at']:
...
pdb.set_trace()
filename='errormodel_SH_cld_1704_12_3kev_para.csv'
with open(filename, 'w', newline='') as file:
    ...
    for r in final_result:
        writer.writerow(r)
filename_l='errormodel_SH_cld_1704_12_3kev_lite.csv'
