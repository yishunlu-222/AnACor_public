...
try:
    ...
    from utils.utils_gridding import *
except:
    from AnACor.utils.utils_rt import *
    from AnACor.utils.utils_ib import *
    from AnACor.utils.utils_gridding import *
import ctypes as ct
                ...
                result = anacor_lib_cpu.ib_test(
                    ...
                    for k , coord in enumerate( coord_list ) :
                        ...
                        if args.bisection_py:
                            it_1,counter_it1=iterative_bisection(theta_1, phi_1, coord, face_1, label_list,num_cls)
                            it_2,counter_it2=iterative_bisection(theta, phi, coord, face_2, label_list,num_cls)
                            numbers_1=np.array(cal_path2_bisection(it_1, voxel_size,face_1) )
                            numbers_2=np.array(cal_path2_bisection(it_2, voxel_size,face_2) )
        ...
        for i, row in enumerate(arr_map):
            ...
            if args.bisection:
                if i == 0:
                        ...
                        print("\033[92m C with {} cores is used for bisection \033[0m".format(args.num_workers))
                result = anacor_lib_cpu.ib_am(
                    ...
                    for k , coord in enumerate( coord_list ) :
                        ...
                        if args.bisection_py:         
                            ...
                            numbers_2=cal_path2_bisection(it_2, voxel_size,face_2) 
...
def worker_function_gridding(t1, low,  dataset,gridding_data, label_list,
                    voxel_size, coefficients, F, coord_list,
                    omega_axis, axes_data, save_dir, args,
                    offset, full_iteration, store_paths, printing,num_cls,gridding_method=2):
    ...
    for i, row in enumerate(arr_map):
        if i == 0:
            print("\033[92m C with {} cores is used for gridding \033[0m".format(args.num_workers))
        rotated_s1 = row
        theta, phi = dials_2_thetaphi((rotated_s1))
        absorption_row = []
        theta_1, phi_1 = 0,0
        ...
        print('[{}/{}] theta: {:.4f}, phi: {:.4f},  absorption: {:.4f}'.format(low + i,up,
                                                                                                          theta * 180 / np.pi,
                                                                                                          phi * 180 / np.pi,
                                                                                                          absorption))
        # pdb.set_trace()
...
def main():
    ...
    if  args.gridding is True:
        ...
        theta = np.linspace(-180, 180, args.gridding_theta,endpoint=False)
        phi = np.linspace(-90, 90, args.gridding_phi,endpoint=False)
        ...
        len_data = len(gridding_data)
        if up == -1:
            gridding_data = gridding_data[low:]
        else:
        ...
        for i in range(num_process):
            ...
            if i != num_process-1:
                ...
                if args.absorption_map is True:
                    ...
                    process=mp.Process(target=worker_function_gridding,
                                        args=(t1, low+i*each_core, dataset,
                                            gridding_data[i*each_core:],  data_copies[i],
                                            voxel_size, coefficients, F, coord_list,
                                            omega_axis, axes_data, gridding_dir, args,
                                            offset, full_iteration, store_paths, printing,num_cls))
                else:
