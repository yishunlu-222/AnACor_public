...
def dials_2_thetaphi_22(rotated_s1,L1=False):
    if L1 is True:
...
@jit(nopython=True)
def cal_coord_2(theta ,phi,coord,face,shape,label_list):
    """
...
def cal_path2_plus(path_2, coord, omega):
    # pdb.set_trace()
    path_ray = path_2[0]
    posi = path_2[1]
    classes = path_2[2]

    # if np.abs(omega) > 1 / 180 * np.pi and np.abs(omega) < 179 / 180 * np.pi :
        # pdb.set_trace()
    cr_l_2 = 0
    lo_l_2 = 0
    li_l_2 = 0
...
def cal_num(coord,path_1,path_2,theta,rotation_frame_angle,plus=True):
# @jit (nopython=True)
    """
    ...
    :return:
    """
    li_l_2,lo_l_2,cr_l_2,bu_l_2=cal_path2_plus(path_2,coord,theta)
    if path_1 is not None:
        li_l_1, lo_l_1, cr_l_1,bu_l_1 = cal_path2_plus(path_1, coord, rotation_frame_angle)
...
def cal_rate_raw(numbers,coefficients , pixel_size):
